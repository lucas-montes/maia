...
///
/// This macro generates a `json_schema()` method that returns a `serde_json::Value`
...
/// ```
#[proc_macro_derive(GeminiSchema, attributes(description, gemini))]
pub fn derive_gemini_schema(input: TokenStream) -> TokenStream {
    ...
    for field in fields {
        ...
        let schema_creation = if is_custom {
            // For custom types, call their json_schema() method
            let type_ident = syn::Ident::new(&type_name, proc_macro2::Span::call_site());
            if is_optional {
                quote! {
                    let mut schema = #type_ident::json_schema();
                    // Make it nullable
                ...
                if let syn::PathArguments::AngleBracketed(args) = &segment.arguments {
                    if let Some(syn::GenericArgument::Type(inner_ty)) = args.args.first() {
                        ...
                        let inner_schema = if is_custom_type {
                            // For nested custom structs, call their json_schema() method
                            let inner_ident = syn::Ident::new(&inner_type_name, proc_macro2::Span::call_site());
                            quote! { #inner_ident::json_schema() }
                        } else {
